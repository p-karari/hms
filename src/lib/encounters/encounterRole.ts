...
export async function getEncounterRoleUuid(roleName: string) {
    ...
    try {
        ...
        if (!response.ok) {
            ...
            throw new Error(`OpenMRS API Error (Encounter Role Lookup): HTTP ${response.status}. Detail: ${errorDetail.substring(0, 100)}...`)
        }
        ...
        if (data.results.length === 0) {
            ...
            throw new Error(`No Encounter Role found matching: ${roleName}. The system configuration may be incorrect.` + `Actual API respponse data: ${responseContent.substring(0, 150)}`);
        }
        ...
        console.error("Error fetching Encounter Role UUID:", error);
        throw new Error(`Could not fetch UUID for Encounter Role: ${roleName}.`)
    }
}