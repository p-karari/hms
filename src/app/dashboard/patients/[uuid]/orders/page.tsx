...
const OrdersPage: React.FC = () => {
  ...
  const [currentEncounterUuid, setCurrentEncounterUuid] = useState<string | null>(null);
  ...
  useEffect(() => {
    const fetchEncounter = async () => {
      try {
        const encounter = await getActiveEncounterUuid(patientUuid);
        setCurrentEncounterUuid(encounter);
      } catch (error) {
        console.error('Failed to fetch active encounter:', error);
      }
    };
    if (patientUuid) fetchEncounter();
  }, [patientUuid, dataVersion]);
  ...
  const encounterId = currentEncounterUuid || 'NO_ACTIVE_ENCOUNTER';
      ...
      <div className="space-y-6 container mx-auto p-4 md:p-8">
        ...
        <div className="bg-white rounded-lg border border-gray-200 p-4">
          <OrdersDashboard
            ...
            currentEncounterUuid={encounterId}
          />
        </div>
      </div>
